...
;; hammin‹——£‚ªdˆÈ‰º‚©
#|
(define (hamming-distance-le-list dna motif k)
  (hamming-distance-list-le-list0 dna motif k))

(define (hamming-distance-list-le-list dnalist motiflist d motiflen )
  (if (= d 0)
      (equal? (take dnalist motiflen) motiflist)
      (if (equal? (car dnlist)(car motiflist))
	  (hamming-distance-list-le0 (cdr dnalist) motiflist d)
	  (hamming-distance-list-le0 (cdr dnalist) motiflist (- d 1))
|#		  
