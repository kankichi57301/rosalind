  ...
  (let* ((data (read-file*
    ...
    (define (makeM n)
      (let* ((m0 (format "M~a" n))
	     ...
	     (d1 (format "D~a" (+ n 1)))
	     (i0 (format "I~a" n))
	     (m0-l (or (find-chars m0)(find-chars n0)))
	     ;;(n0-l (find-chars n0))
	     (m1-l (or (find-chars m1)(find-chars n1)))
	     (i0-l (find-chars i0))
	     (n1-l (find-chars n1))
	     )

	;(displayln (format "c:main:=~a" m0-l))
	;(displayln (format "n:main:=~a" m1-l))
	(if (not i0-l)  ;; Iがない場合
	  (begin  
	    ...
	    (let* ((m0pat (if (= 0 n)
		   ...
		   (m1pat (if (= seqlen n)
			      ...
			      (notgap1 (car m1-l))))
		   (m0cnt (apply + m0pat))
		   (MMcnt (apply + (map bitwise-and m0pat m1pat)))
		   (MMratio (* 1.0 (/ MMcnt m0cnt)))
		   )
	      (displayln (format "M~a pat=~a next pat=~a mmratio=~a" n m0pat m1pat (roundp3 MMratio)))
	    ))
...
;; I state からの遷移
    ...
    (define (emit-1line from)
      (cons from (map roundp3 (hash-ref *emit* from (make-list sym-cnt 0.0)))))
