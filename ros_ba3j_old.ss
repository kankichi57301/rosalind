...
(define (merge-all strlist)
  (merge-all0 (car strlist)(cdr strlist) 1))

(define (merge-all0 accstr reststrlist cnt)
    (if (null? reststrlist)
      accstr
	...
	(if next
	    (begin
	      ...
	      (merge-all0 (merge-gapped0 accstr next 1)(delete-once next reststrlist)(+ 1 cnt)))
	    (let ((next2 (find-first (lambda(x)(str-shift-match accstr x -1)) reststrlist)))
	      (when next2
		    (displayln (format "next- =[~a] acc=[~a]" cnt (string-length accstr)))
		    (merge-all0 (merge-gapped0 accstr next2 -1)(delete-once next2 reststrlist)(+ 1 cnt))))))))
