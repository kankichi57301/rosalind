...
(define *res* '()) 
(define (solve-ksim-align str1 str2 k)
 (let ((len1 (string-length str1))
   ...
   (set! *len2* len2)
    ...
    (for-each (lambda(x)
		(when (<= (- (dp-ref     len1 x len2)
			     (traceback  len1 x len2))
			  k)
		(set! *res* (cons `(,*y-min* ,x) *res*))))
    (iota len2 1))
    (reverse *res*)
    |#
    ))
...
(define (tracebackB x y len2 k acc )
  ...
  (let ((nextacc (if (<= x k)
	...
	(case (tb-ref x y len2)
	      ...
	      [(0) (tracebackB (- x 1)(- y 1) len2 k nextacc  )]
...
(define s5 "ABCD")
(define s6 "ABCDABCD")
(solve-ksim-align s5 s6 2)

;(define (traceback x y k)
;  (tracebackA x y k *len2* '() "" ""))
