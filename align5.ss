...
(define (init-dp-3d len1 len2 len3)
  ...
  (hash-set! *dp*  '(0 0 0) 0)
  (for-each (lambda(x)(hash-set! *dp*  `(,x 0 0) (* 2 x)))(iota (+ 1 len1)))
  (for-each (lambda(y)(hash-set! *dp*  `(0 ,y 0) (* 2 y)))(iota (+ 1 len2)))
  (for-each (lambda(z)(hash-set! *dp*  `(0 0 ,z) (* 2 z)))(iota (+ 1 len3)))
  ...
  (for-each
   (lambda(x)
	...
	(let* ((z 0)
	       ...
	       (v1   (+ 2    (hash-ref *dp* `(,(- x 1) ,y ,z))))
	       (v2   (+ 2    (hash-ref *dp* `(,x ,(- y 1) ,z))))
	       (v3   (+ 2    *INF*))
